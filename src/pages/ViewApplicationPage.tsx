    ...
    <PortalLayout>
      ...
      <div className="sticky top-[64px] z-30 bg-white/90 backdrop-blur-md border-b border-[#E5E7EB]">
        <div className="max-w-3xl mx-auto px-6 py-3 flex items-center justify-between">
          <Link 
            ...
            className="flex items-center gap-1.5 text-xs font-black text-[#3538CD] hover:text-[#292bb0] transition-all uppercase tracking-widest"
          >
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-black text-[#111827] tracking-tight mb-2">{appData.jobTitle}</h1>
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-[#6B7280] font-medium">
              <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5" />{appData.location}</span>
              <span className="text-[#D1D5DB]">·</span>
              <span className="flex items-center gap-1"><Briefcase className="w-3.5 h-3.5" />{appData.employmentType}</span>
              <span className="text-[#D1D5DB]">·</span>
              <span className="flex items-center gap-1"><Building2 className="w-3.5 h-3.5" />{appData.jobType}</span>
              <span className="text-[#D1D5DB]">·</span>
              <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" />{appData.experience}</span>
            </div>
          </div>
          {canWithdraw && (
            <button 
              onClick={() => setIsWithdrawModalOpen(true)}
              className="shrink-0 flex items-center gap-1.5 px-4 py-2 border border-[#E5E7EB] text-[#6B7280] hover:text-red-500 hover:border-red-200 hover:bg-red-50 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all"
            >
        ...
        <div className="space-y-2">
          ...
          <AccordionCard title="Job Summary" defaultExpanded={false}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                <p className="text-[10px] font-black text-[#9CA3AF] uppercase tracking-widest mb-2">Requirements</p>
                <ul className="space-y-1.5">
                  {appData.requirements.map((req, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-[#4B5563]">
                      <div className="w-1 h-1 rounded-full bg-[#3538CD] mt-2 shrink-0" />
                      {req}
                    </li>
                  ))}
                </ul>
          ...
          <AccordionCard title="Personal Profile">
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
              ...
              <div className="col-span-2 flex items-center gap-2 text-sm text-[#3538CD] font-medium pt-1">
          ...
          <AccordionCard title="Additional Information">
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
              <div className="col-span-2">
                <p className="text-[10px] font-black text-[#9CA3AF] uppercase tracking-widest mb-1">Portfolio URL</p>
                <a href={appData.additionalInfo.portfolioUrl} target="_blank" rel="noreferrer"
                  className="text-sm font-medium text-[#3538CD] hover:underline flex items-center gap-1.5">
          ...
          <AccordionCard title="Resume">
            <div className="flex items-center gap-4 py-1">
              <div className="w-10 h-10 rounded-xl bg-[#F4F5FA] flex items-center justify-center text-[#3538CD] border border-[#E5E7EB] shrink-0">
                <FileText className="w-5 h-5" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-[#111827] truncate">{appData.resume.filename}</p>
                <p className="text-[10px] text-[#9CA3AF] font-medium uppercase tracking-widest">PDF · 2.4 MB</p>
              </div>
              <button className="flex items-center gap-1.5 px-4 py-2 bg-white border border-[#E5E7EB] rounded-xl text-xs font-black text-[#6B7280] hover:text-[#3538CD] hover:border-[#3538CD]/30 transition-all uppercase tracking-widest">
      ...
      {isWithdrawModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-[#E5E7EB] animate-in fade-in zoom-in duration-200">
            <div className="p-7">
              <div className="w-12 h-12 rounded-2xl bg-red-50 text-red-600 flex items-center justify-center mb-5">
                <AlertTriangle className="w-6 h-6" />
              </div>
              <h3 className="text-xl font-black text-[#111827] tracking-tight mb-2">Withdraw Application?</h3>
              ...
              <div className="flex gap-3">
                <button 
                  ...
                  className="flex-1 px-5 py-3 bg-[#F9FAFB] text-[#111827] text-xs font-black rounded-xl hover:bg-[#F3F4F6] transition-all uppercase tracking-widest border border-[#E5E7EB]"
                >
                  Cancel
                </button>
                <button 
                  ...
                  className="flex-1 px-5 py-3 bg-red-600 text-white text-xs font-black rounded-xl hover:bg-red-700 transition-all uppercase tracking-widest"
                >
                  Yes, Withdraw
                </button>
              </div>
...
function AccordionCard({ title, children, defaultExpanded = true }: { title: string; children: React.ReactNode; defaultExpanded?: boolean }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E5E7EB] overflow-hidden">
      ...
      >
        <span className="text-[11px] font-black text-[#374151] uppercase tracking-widest">{title}</span>
...
function InfoItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="space-y-1">
      ...
      <div className="text-sm font-semibold text-[#111827]">{value || <span className="text-[#D1D5DB] font-normal">Not provided</span>}</div>
    </div>
  );
}
